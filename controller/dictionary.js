...
class DictionaryController{
    static async List (ctx){
        ...
        const info = await models.Dictionary.findAndCountAll({
            where:Filter,
    ...
    static async Add (ctx){
        ...
        const Info = await models.Dictionary.create({
    ...
    static async Edit (ctx){
        ...
        const updateStatus =  await models.Dictionary.update({
            type, key, value,OrganizationalId:organizational_id
        },{
            where:{
                id
        ...
        if(updateStatus[0]>0){
            const returnUpdated =  await models.Dictionary.findOne({
                where:{
                    id
                },
                attributes: [
